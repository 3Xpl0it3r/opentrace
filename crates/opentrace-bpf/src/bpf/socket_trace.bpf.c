...
enum role_type { ROLE_CLIENT, ROLE_SERVER };
...
static __always_inline void populate_conn_info(const struct sock *sk,
  ...
  sock_daddr(sk, 0x00, &value->daddr);
  sock_saddr(sk, 0, &value->saddr);
  sock_sport(sk, &value->sport);
  sock_dport(sk, &value->dport);
  value->family = family;
  if (role == ROLE_CLIENT)
    value->role = 0;
  else
...
process_syscall_exit(struct trace_event_raw_sys_exit *ctx,
  ...
  bpf_printk("event size: %d, %ld", event->size, event->timestamp);
...
int tp_sys_exit_connect(struct trace_event_raw_sys_exit *ctx) {
  ...
  struct conn_info_value value = {};
  struct conn_info_key key = {.tgid = tgid, .fd = args->fd};
  populate_conn_info(args->sk, 0, ROLE_CLIENT, &value);
...
int tp_sys_enter_accept(struct trace_event_raw_sys_enter *ctx) {
  ...
  struct syscall_accept_args_t args = {.fd = 0, .sk = NULL, .addr = uservadd};
...
int kret_sock_alloc(struct pt_regs *ctx) {
  ...
  bpf_probe_read_kernel(&args->sk, sizeof(args->sk),
                        (void *)skt + offsetof(struct socket, sk));
  return BPF_OK;
}
...
int tp_sys_exit_accept(struct trace_event_raw_sys_exit *ctx) {
  ...
  populate_conn_info(args->sk, 0, ROLE_SERVER, &value);
  bpf_map_update_elem(&conn_info_map, &key, &value, BPF_ANY);
...
// ---------------------------------------------------------------------------
//       tcp send 获取socket读取的业务数据，通过perf_event 发送给用户态
// ---------------------------------------------------------------------------
...
int tp_sys_enter_close(struct trace_event_raw_sys_enter *ctx) {
  ...
  u64 fd = ctx->args[0];
  // 直接从active_conn_info_maps里面删除记录
  return BPF_OK;
}
