...
macro_rules! define_collector {
    ($name:ident, $skel:ident) => {
        ...
        impl<'a> $crate::collector::Collector for $name<'a> {
            ...
            fn attach_probe(
                &mut self,
                probe_registry: &$crate::ProbeRegistry,
            ) -> ::std::result::Result<(), $crate::EbpfError> {
                // 这个需要用户自己实现
                self.do_attach_probes(probe_registry)
            }
        }
    };
}
...
macro_rules! attach_tracepoint {
    ($self:ident, $registry:ident, $cat:literal, $prog:ident) => {{
        ...
        if $registry.tracepoint_is_available(&lookup) {
            let link = $self
                .skel
                .progs
                .$prog
...
// attach_multiple_tracepoints!(self, probe_registry, "syscalls", tp_sys_enter, ["sys_enter_connect", "sys_enter_accept"]);
macro_rules! attach_multiple_tracepoints {
    ($self:ident, $registry:ident, $cat:literal, $prog:ident, $tps:expr) => {{
        let (category, prefix) = match $cat {
            "syscalls" => (::libbpf_rs::TracepointCategory::Syscalls, "syscalls"),
            "net" => (::libbpf_rs::TracepointCategory::Net, "net"),
            other => panic!(
...
macro_rules! attach_kprobe {
    ($self:ident, $registry:ident, $prog:ident, $func:expr) => {{
        let func = $func;
        if $registry.kprobe_is_available(func) {
